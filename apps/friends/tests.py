...
class TestFriendRequests(APITestCase):
    ...
    def test_friend_request_create(self):
        friend1 = UserFactory.create(username="friend1")
        friend2 = UserFactory.create(username="friend2")
        application = ApplicationFactory.create()
        access_token = AccessTokenFactory.create(
            user=friend1, application=application
        )
        ...
        self.assertTrue(
            FriendRequest.objects.filter(
                from_user=friend1, to_user=friend2, accepted=False
            ).exists()
        )
    ...
    def test_friend_request_accept(self):
        ...
        friend_request = FriendRequestFactory.create(
            from_user=friend1, to_user=friend2
        )
        ...
        url = reverse(
            "friendrequest-accept-request", kwargs={"pk": friend_request.id}
        )
        ...
        access_token = AccessTokenFactory.create(
            user=friend2, application=application
        ).token
        ...
        self.assertTrue(
            FriendRequest.objects.filter(
                from_user=friend1, to_user=friend2, accepted=True
            ).exists()
        )
        self.assertTrue(
            Friend.objects.filter(user=friend1, friend=friend2).exists()
        )
    ...
    def test_friend_request_reject(self):
        ...
        url = reverse(
            "friendrequest-reject-request", kwargs={"pk": friend_request.id}
        )
...
class TestFriends(APITestCase):
    ...
    def test_add_close_friend(self):
        ...
        self.assertTrue(
            Friend.objects.filter(
                user=user1, friend=user2, is_close_friend=True
            ).exists()
        )
    ...
    def test_remove_close_friend(self):
        ...
        url = reverse(
            "friends-remove-close-friend", kwargs={"pk": friendship.id}
        )
        application = ApplicationFactory.create()
        access_token = AccessTokenFactory.create(
            user=user1, application=application
